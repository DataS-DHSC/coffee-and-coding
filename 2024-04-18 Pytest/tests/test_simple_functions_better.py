...
def test_add(test_x, test_y, expected_output):
    assert (
        add(x=test_x, y=test_y) == expected_output
    ), f"add({test_x}, {test_y}) should return {expected_output}"


def test_wait_1000_seconds_then_multiply(mocker):
    """
...
def test_wait_then_add(mocker, t):
...
@pytest.mark.parametrize(
    "x, y",
    [(0.0, 1), (0, 1.5), (1.5, 0), (2.3, 1.2), (-0.1, 1), ("s", 1), ("1", "2")],
)
def test_errors(x, y):
    """
    add() should only run if both arguments are integers, and through a
